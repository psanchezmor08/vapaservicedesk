...
# --- 2. CONFIGURACIÓN DE CORREO Y LDAP (YAGMAIL + .ENV) ---
...
LDAP_SERVER_URL = os.getenv("LDAP_SERVER", "ldap://localhost")
LDAP_PORT       = int(os.getenv("LDAP_PORT", "389"))
LDAP_BASE_DN    = os.getenv("LDAP_BASE_DN", "dc=vapa,dc=es")
LDAP_BIND_DN    = os.getenv("LDAP_BIND_DN", "cn=admin,dc=vapa,dc=es")
...
def init_db():
    with sqlite3.connect(DB_PATH) as conn:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS usuarios
            (username TEXT PRIMARY KEY, password TEXT, rol TEXT, estado TEXT, email TEXT, debe_cambiar_pass INTEGER DEFAULT 0)''')
...
# --- 6. AUTENTICACIÓN LDAP ---
def login_ldap(username, password):
    """
    Autentica un usuario contra OpenLDAP (dc=vapa,dc=es).
    Busca en ou=users,dc=vapa,dc=es por el atributo uid.
    Devuelve (rol, debe_cambiar_pass) o (None, None) si falla.
    """
    try:
        server = Server(LDAP_SERVER_URL, port=LDAP_PORT, get_info=ALL)

        # Bind de servicio (admin) para buscar al usuario
        ...
        # Buscar el DN del usuario bajo ou=users,dc=vapa,dc=es
        admin_conn.search(
        ...
        if not admin_conn.entries:
            return (None, None)  # Usuario no existe en LDAP

        user_dn   = admin_conn.entries[0].entry_dn
        user_mail = str(admin_conn.entries[0].mail) if admin_conn.entries[0].mail else f"{username}@vapa.es"

        # Bind con las credenciales reales del usuario para verificar contraseña
        user_conn = Connection(server, user=user_dn, password=password, auto_bind=True)

        if not user_conn.bound:
            return (None, None)  # Contraseña incorrecta

        # Autenticación OK — buscar o crear en BD local
        result = run_query("SELECT rol, debe_cambiar_pass FROM usuarios WHERE username = ?", (username,))
        if result:
            return (result[0][0], result[0][1])
        else:
            # Primera vez que entra vía LDAP: crear en BD local como Usuario
            run_query(
                "INSERT INTO usuarios VALUES (?,?,?,?,?,?)",
                (username, "LDAP_AUTH", "Usuario", "Activo", user_mail, 0)
            )
            return ("Usuario", 0)
...
def login(username, password):
    """
    Login híbrido:
    1. Intenta autenticar contra LDAP (OpenLDAP en vapa.es)
    2. Si falla, usa el login local por BD (para admin y usuarios sin LDAP)
    """
    # Paso 1: intentar LDAP
    ldap_result = login_ldap(username, password)
    if ldap_result[0]:
        return ldap_result

    # Paso 2: fallback local
    result = run_query(
        "SELECT password, rol, debe_cambiar_pass FROM usuarios WHERE username = ?", (username,)
    )
    if result:
        db_hash, rol, debe_cambiar = result[0]
        if db_hash != "LDAP_AUTH" and check_pass(password, db_hash):
            return (rol, debe_cambiar)

    return (None, None)

# --- 7. FUNCIONES AUXILIARES (CORREO, EXPORTACIÓN, PDF) ---
...
def seccion_chat_incidencia(id_inc, autor_reporte, tecnico_asig):
    ...
    if nuevo_comentario:
        ...
        if autor_actual == autor_reporte and tecnico_asig != "Sin Asignar":
            ...
            if mail_tec:
                disparar_correo_async(mail_tec[0][0], f"Comentario Usuario - Ticket #{id_inc}", f"{autor_actual} dice: {nuevo_comentario}")
        elif autor_actual != autor_reporte:
            mail_usr = run_query("SELECT email FROM usuarios WHERE username = ?", (autor_reporte,))
            if mail_usr:
                disparar_correo_async(mail_usr[0][0], f"Respuesta Técnica - Ticket #{id_inc}", f"El técnico {autor_actual} ha respondido.")
        st.rerun()
            ...
            if st.form_submit_button("💾 Guardar Cambios"):
                ...
                if nueva_prio != prioridad:
                    ...
                    try:
                        ...
                        pass
                run_query(
                    "UPDATE incidencias SET informe_tecnico=?, estado=?, prioridad=?, fecha_limite=?, fecha_resolucion=? WHERE id=?",
                    (n_informe, n_estado, nueva_prio, n_limite, n_resolucion, inc[0])
                )
                if n_estado != inc[5]:
                    m_usr = run_query("SELECT email FROM usuarios WHERE username = ?", (inc[3],))
                    if m_usr:
                        disparar_correo_async(m_usr[0][0], f"Estado Ticket #{inc[0]}", f"Estado actualizado a: {n_estado}")
...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state['logged_in']:
            st.subheader("Login")
            st.caption("🔐 Usa tus credenciales LDAP o locales")
            ...
            if st.button("Entrar"):
                ...
                if d_login[0]:
                    st.session_state.update({
                        ...
                        'rol': d_login[0], 'debe_cambiar_pass': d_login[1],
        ...
        if st.session_state['debe_cambiar_pass'] == 1:
            ...
            with st.form("f_pass"):
                ...
                if st.form_submit_button("Actualizar"):
                    if p1 and p1 == p2:
                        run_query(
                            "UPDATE usuarios SET password=?, debe_cambiar_pass=0 WHERE username=?",
            ...
            if st.session_state['rol'] == 'Administrador':
                ...
                st.info("💡 Los usuarios LDAP aparecen con contraseña 'LDAP_AUTH'. Su autenticación siempre va contra OpenLDAP.")
                ...
                if st.button("Guardar Usuarios"):
                    for _, r in e_df.iterrows():
                        ...
                        if run_query("SELECT username FROM usuarios WHERE username=?", (u,)):
                            ...
                            # Solo resetear contraseña si NO es un usuario LDAP
                            if res:
                                pwd_actual = run_query("SELECT password FROM usuarios WHERE username=?", (u,))
                                if pwd_actual and pwd_actual[0][0] != "LDAP_AUTH":
                                    run_query("UPDATE usuarios SET password=?, debe_cambiar_pass=1 WHERE username=?",
                                              (hash_pass("1234"), u))
                                else:
                                    st.warning(f"'{u}' es usuario LDAP, no se puede resetear la contraseña desde aquí.")
                        else:
            ...
            if st.session_state['incidencia_seleccionada']:
                    ...
                    with st.form("f_inc"):
                        ...
                        with col2:
                            ...
                            m_tec  = "Sin Asignar" if st.session_state.rol == 'Usuario' else \
                                     st.selectbox("Técnico", ["Sin Asignar"] + [t[0] for t in run_query("SELECT username FROM usuarios WHERE rol='Tecnico'")])
                        ...
                        if st.form_submit_button("Registrar Ticket"):
                            if m_tit and m_id:
                                if run_query("SELECT id FROM incidencias WHERE id=?", (m_id,)):
                                    ...
                                    run_query(
                                        "INSERT INTO incidencias (id, titulo, descripcion, usuario_reporte, tecnico_asignado, estado, fecha, archivo, prioridad, categoria, fecha_limite) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                ...
                with tabs[3]:
                    ...
                    with st.expander("🔑 Restablecer Pass"):
